...
const BACKEND_URL = "https://script.google.com/macros/s/AKfycbvV5iYwbY8xBoMnki_N8qKosRk2mu9kukqm8Hqg4quYT6OtFLJyYiQi_rnXTEdjzTr9/exec";
...
const Register = () => {
  ...
  const handleFormSubmissionEvent = async (e) => {
    ...
    try {
      ...
      const payloadBundle = {
        ...
        anuragian: formData.college && (formData.college.trim().toLowerCase().replace(/\s+/g, ' ') === "anurag university" || formData.college.trim().toLowerCase().replace(/\s+/g, ' ') === "au") ? "YES" : "NO",
      };
  ...
  const getNormalizedCollege = () => {
    return formData.college ? formData.college.trim().toLowerCase().replace(/\s+/g, " ") : "";
  };

  const isEarlyBirdEligible = () => {
    const normalized = getNormalizedCollege();
    return normalized === "anurag university" || normalized === "au";
  };

  const hasEnteredCollege = () => {
        ...
        {viewStateMode === "form" ? (
                ...
                <div className="flex flex-col gap-6 mt-2">
                  ...
                  {!hasEnteredCollege() && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      className="bg-blue-50/50 border border-blue-200 rounded-3xl p-6 text-center shadow-sm"
                    >
                      <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-blue-100 text-blue-600 mb-4">
                        <Info size={24} />
                      </div>
                      <h3 className="text-sm font-bold text-slate-800 mb-2">Registration Fee Information</h3>
                      <p className="text-xs font-medium text-slate-500">
                        Please enter your Institution / College to view the applicable registration fee.
                      </p>
                    </motion.div>
                  )}

                  {/* Card 1: Early Bird */}
                  {hasEnteredCollege() && isEarlyBirdEligible() && (
                    <motion.div
          ...
          >
            ...
            <div className="pt-4">
              ...
              >
                <b>↩ Lodge Another Registration</b>
              </button>
            </div>
          </motion.div>
        )}
